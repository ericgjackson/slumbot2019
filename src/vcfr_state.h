...
class VCFRState {
 public:
  VCFRState(double *opp_probs, int **street_buckets, const HandTree *hand_tree);
  VCFRState(double *opp_probs, const HandTree *hand_tree,  int lbd,
	    const std::string &action_sequence, int root_bd, int root_bd_st,
	    int **street_buckets);
  VCFRState(double *opp_probs, double *total_card_probs, const HandTree *hand_tree, int st, int lbd,
	    const std::string &action_sequence, int root_bd, int root_bd_st, int **street_buckets);
  VCFRState(const VCFRState &pred, Node *node, int s);
  VCFRState(const VCFRState &pred, Node *node, int s, double *opp_probs, double sum_opp_probs,
