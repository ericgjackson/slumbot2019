...
public:
  SubgameSolver(const CardAbstraction &base_card_abstraction,
		...
		unsigned int num_subgame_its, unsigned int num_threads);
  ~SubgameSolver(void) {}
...
private:
  ...
  void Split(Node *node, const string &action_sequence, unsigned int pgbd,
	     double **reach_probs);
...
SubgameSolver::SubgameSolver(const CardAbstraction &base_card_abstraction,
  ...
  subgame_betting_abstraction_(subgame_betting_abstraction),
  base_cfr_config_(base_cfr_config), subgame_cfr_config_(subgame_cfr_config),
  base_buckets_(base_buckets), subgame_buckets_(subgame_buckets) {
  solve_st_ = solve_st;
  method_ = method;
  cfrs_ = cfrs;
  ...
  trunk_sumprobs_.reset(new CFRValues(nullptr, trunk_streets.get(), 0, 0,
				      base_buckets_,
  ...
  if (base_mem_) {
    ...
    dynamic_cbr_.reset(new DynamicCBR(base_card_abstraction_,
				      base_betting_abstraction_,
				      base_cfr_config_, base_buckets_,
				      base_betting_tree_.get(), 1));
    if (current_) {
	...
	subgame_streets[st] = st >= solve_st_;
      ...
      regrets.reset(new CFRValues(nullptr, subgame_streets.get(), 0, 0,
				  base_buckets_, base_betting_tree_.get()));
...
BettingTree *SubgameSolver::CreateSubtree(Node *node, unsigned int target_p,
					  bool base) {
...
class SSThread {
public:
  SSThread(Node *node, const string &action_sequence, unsigned int pgbd,
	   double **reach_probs, SubgameSolver *solver,
...
SSThread::SSThread(Node *node, const string &action_sequence,
		   unsigned int pgbd, double **reach_probs,
		   SubgameSolver *solver, unsigned int thread_index,
		   unsigned int num_threads) :
...
void SubgameSolver::Split(Node *node, const string &action_sequence,
			  unsigned int pgbd, double **reach_probs) {
  unique_ptr<SSThread * []> threads(new SSThread *[num_threads_]);
  for (unsigned int t = 0; t < num_threads_; ++t) {
    threads[t] = new SSThread(node, action_sequence, pgbd, reach_probs, this,
			       t, num_threads_);
  }
...
void SubgameSolver::StreetInitial(Node *node, const string &action_sequence,
				  unsigned int pgbd, double **reach_probs) {
...
void SubgameSolver::Resolve(Node *node, unsigned int gbd,
  ...
  if (method_ == ResolvingMethod::UNSAFE) {
    eg_cfr.reset(new UnsafeEGCFR(subgame_card_abstraction_,
				 base_card_abstraction_,
				 subgame_betting_abstraction_,
				 base_betting_abstraction_,
				 subgame_cfr_config_, base_cfr_config_,
				 subgame_buckets_, 1));
  } else {
  ...
  for (unsigned int asym_p = 0; asym_p < num_asym_players; ++asym_p) {
    ...
    if (! base_mem_) {
      ...
      unique_ptr<CFRValues> base_subgame_strategy(
	    ReadBaseSubgameStrategy(base_card_abstraction_,
				    base_betting_abstraction_,
				    base_cfr_config_, base_betting_tree_.get(),
				    base_buckets_, subgame_buckets_, base_it_,
    ...
    if (method_ == ResolvingMethod::UNSAFE) {
      ...
      eg_cfr->SolveSubgame(subgame_subtree, gbd, reach_probs, action_sequence,
			   &hand_tree, nullptr, kMaxUInt, true,
			   num_subgame_its_);
      // Write out the P0 and P1 strategies
      for (unsigned int solve_p = 0; solve_p < num_players; ++solve_p) {
	WriteSubgame(subgame_subtree->Root(), action_sequence, action_sequence,
		     gbd, base_card_abstraction_, subgame_card_abstraction_,
		     base_betting_abstraction_, subgame_betting_abstraction_,
		     base_cfr_config_, subgame_cfr_config_, method_,
	...
	// each player.
	eg_cfr->SolveSubgame(subgame_subtree, gbd, reach_probs,
			     action_sequence, &hand_tree, t_vals.get(),
			     solve_p, false, num_subgame_its_);
...
void SubgameSolver::Walk(Node *node, const string &action_sequence,
...
static void Usage(const char *prog_name) {
  fprintf(stderr, "USAGE: %s <game params> <base card params> "
	  "<subgame card params> <base betting params> "
	  "<subgame betting params> <base CFR params> <subgame CFR params> "
	  "<solve street> <base it> <num subgame its> "
  ...
  fprintf(stderr, "\"current\" or \"avg\" signifies whether we use the "
	  "opponent's current strategy (from regrets) in the subgame CBR "
  ...
  fprintf(stderr, "\"mem\" or \"disk\" signifies whether the base strategy "
	  "for the subgame streets is loaded into memory at startup, or "
	  "whether we read the base subgame strategy as needed.  Note that "
...
int main(int argc, char *argv[]) {
  ...
  unsigned int solve_st, base_it, num_subgame_its;
  if (sscanf(argv[8], "%u", &solve_st) != 1)         Usage(argv[0]);
  if (sscanf(argv[9], "%u", &base_it) != 1)         Usage(argv[0]);
  if (sscanf(argv[10], "%u", &num_subgame_its) != 1) Usage(argv[0]);
  string m = argv[11];
  ...
  SubgameSolver solver(*base_card_abstraction, *subgame_card_abstraction,
		       *base_betting_abstraction,
		       *subgame_betting_abstraction, *base_cfr_config,
		       *subgame_cfr_config, base_buckets, subgame_buckets,
		       solve_st, method, cfrs, card_level, zero_sum, current,
		       pure_streets.get(), base_mem, base_it, num_subgame_its,
		       num_threads);
  for (unsigned int asym_p = 0; asym_p <= 1; ++asym_p) {
    DeleteAllSubgames(*base_card_abstraction, *subgame_card_abstraction,
		      *base_betting_abstraction, *subgame_betting_abstraction,
		      *base_cfr_config, *subgame_cfr_config, method, asym_p);
  }
  solver.Walk();
}
