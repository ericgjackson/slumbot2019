...
MPECFRThread::MPECFRThread(const CFRConfig &cfr_config, const Buckets &buckets, MPECFRNode *root,
...
double MPECFRThread::Process(MPECFRNode *node) {
  if (node->Terminal()) {
...
static void *thread_run(void *v_t) {
  MPECFRThread *t = (MPECFRThread *)v_t;
  t->Run();
  return NULL;
}

void MPECFRThread::RunThread(void) {
  pthread_create(&pthread_id_, NULL, thread_run, this);
}

void MPECFRThread::Join(void) {
  pthread_join(pthread_id_, NULL); 
}

void MPECFR::ReadRegrets(MPECFRNode *node, unique_ptr<Reader> *readers) {
...
void MPECFR::ReadSumprobs(MPECFRNode *node, unique_ptr<Reader> *readers) {
  if (node->Terminal()) return;
  int max_street = Game::MaxStreet();
  int st = node->Street();
  int pa = node->PlayerActing();
...
void MPECFR::Read(int batch_index) {
...
void MPECFR::WriteRegrets(MPECFRNode *node, unique_ptr<Writer> *writers) {
  if (node->Terminal()) return;
  int max_street = Game::MaxStreet();
  int st = node->Street();
  int pa = node->PlayerActing();
...
void MPECFR::WriteSumprobs(MPECFRNode *node, unique_ptr<Writer> *writers) {
  if (node->Terminal()) return;
  int max_street = Game::MaxStreet();
  int st = node->Street();
  int pa = node->PlayerActing();
...
void MPECFR::Write(int batch_index) {
  char dir[500], buf[500];
...
void MPECFR::RunBatch(int batch_index, int batch_size) {
  srand48_r(batch_index, &rand_buf_);
  cfr_threads_.reset(new unique_ptr<MPECFRThread> [num_cfr_threads_]);
...
#endif
    int seed = 0;
    cfr_threads_[t].reset(new MPECFRThread(cfr_config_, buckets_, root_.get(), seed, batch_size,
...
void MPECFR::Run(int start_batch_index, int end_batch_index, int batch_size, int save_interval) {
...
MPECFR::MPECFR(const CardAbstraction &ca, const BettingAbstraction &ba, const CFRConfig &cc,
  ...
  // int *stack_sizes);
  // Need to pass in remaining and other arguments
  int num_players = Game::NumPlayers();
  unique_ptr<bool []> folded(new bool[num_players]);
  unique_ptr<bool []> active(new bool[num_players]);
  unique_ptr<int []> contributions(new int[num_players]);
  unique_ptr<int []> stack_sizes(new int[num_players]);
  ...
  for (int p = 0; p < num_players; ++p) {
    ...
    // For now
    stack_sizes[p] = ba.StackSize();
  }
  root_.reset(new MPECFRNode(betting_trees.Root(), buckets_, folded.get(), active.get(),
			     contributions.get(), stack_sizes.get()));
